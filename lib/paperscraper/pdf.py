...
def save_pdf_from_dump(dump_path: str, pdf_path: str, key_to_save: str = "doi") -> None:
    ...
    for i, paper in enumerate(pbar):
        ...
        if "doi" not in paper.keys() or paper["doi"] is None:
            ...
            # [Added]
            # For some cases, doi data have multiple dois, select the first one.
            list_of_lines = paper["doi"].split('\n')
            paper["doi"] = str(list_of_lines[0])
            # [Added-End]

        filename = paper[key_to_save].replace("/", "_")

        # [Added]
        new_file_path = os.path.join(pdf_path, f"{filename}.pdf")
        check_file = os.path.exists(new_file_path)

        if check_file:
            continue
        # [Added-End]

        save_pdf(paper, new_file_path)
